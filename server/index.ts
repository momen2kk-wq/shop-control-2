...
app.post("/api/sales",auth,(req:any,res)=>{
 const {productId,qty=1,payment="Cash",customerName="",customerPhone=""}=req.body||{};
 ...
 const tx=db.transaction(()=>{
  ...
  if(customerName){
   ...
   if(c)db.prepare("UPDATE customers SET spent=spent+?,orders=orders+1,last_purchase=?,phone=COALESCE(NULLIF(?,''),phone) WHERE id=?").run(total,date,customerPhone||"",c.id);
   else db.prepare("INSERT INTO customers(id,shop_id,name,phone,spent,orders,last_purchase) VALUES(?,?,?,?,?,?,?)").run(id(),req.user.shopId,customerName,customerPhone||"",total,1,date);
  }
 });tx();
